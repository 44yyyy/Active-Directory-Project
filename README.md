...
This project is designed to build and deploy a security monitoring and response environment for user account management on Windows Active Directory. It begins with general planning with network infrastructure and is followed by deploying virtual machines in the cloud, configuring firewall rules, and establishing a Windows Active Directory domain with a test user. From the machines, telemetry would be forwarded to a Splunk instance, where the information would go through a configured alert to identify unauthorized login activity. To extend the workflow, Slack and Shuffle (SOAR) will be integrated to build an automated response process that would alert a team of analysts and allow them to disable compromised accounts directly through a user prompt. Overall, this project reinforced my practical understanding essential domains for modern security operations, including cloud infrastructure, log ingestion, SIEM tuning, and real-time security automation.
